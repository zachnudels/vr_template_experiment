...
public class ViveEyeDevice
{
    ...
    void Worker()
    {
        ...
        try
        {
            while (running)
            {
                ...
                {
                    //UnityEngine.Debug.Log("SRanipal not running");
                    //UnityEngine.Debug.Log(SRanipal_Eye_Framework.Status.ToString());
                    continue;
                }
    ...
    public void StopDevice()
    {
        ...
        try
        {
            // Wait for the task to finish (optional timeout for safety)
            if (eyeTrackerTask != null && !eyeTrackerTask.IsCompleted)
            {
                bool completed = eyeTrackerTask.Wait(500); // waits up to 500ms
                if (!completed)
                {
                    UnityEngine.Debug.LogWarning("Eye tracker task did not finish within timeout.");
                }
            }
        }
        catch (AggregateException ae)
        {
            foreach (var e in ae.InnerExceptions)
            {
                if (!(e is OperationCanceledException))
                    UnityEngine.Debug.LogError("Exception in eye tracker task: " + e);
            }
        }
